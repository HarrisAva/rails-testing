...
RSpec.describe 'Users API', type: :request do
  describe 'POST /users' do
    let(:valid_attributes) { { name: 'Alex', email: 'alex@email.com' } }
    ...
    context 'when the request is valid' do
      ...
      it 'returns the created user' do
        ...
        expect(json['email']).to eq('alex@email.com')
      end

      it 'saves the user with the correct attributes' do
        user = User.last
        expect(user.name).to eq('Alex')
        expect(user.email).to eq('alex@email.com')
      end
  ...
  describe 'GET /users' do
    let!(:users) { create_list(:user, 10) } # create users by Factory Bot
  ...
  describe 'PUT /users/:id' do
    ...
    context 'when the request is valid' do
      before { put "/users/#{user.id}", params: { user: { name: 'John', email: 'john@gmail.com' } } }
