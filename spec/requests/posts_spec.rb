...
RSpec.describe 'Posts API', type: :request do  # type is request

  describe 'POST /posts' do  # describe action as POST to create a post
    let(:valid_attributes) { { title: 'My first post', content: 'Content of the post' } }
    # use let to define valid_attributes variable, with a hash 'title' and 'content' attributes

    context 'when the request is valid' do
        before { post '/posts', params: { post: valid_attributes } }
      # Context is a way to group tests together
      # runs the code before each test, making a POST request to the /posts endpoint with the valid_attributes hash.
...
describe 'GET /posts' do
  ...
  describe 'GET /posts/:id' do
    ...
    # declare post variable and create a new post using factory-bot's method called create.
